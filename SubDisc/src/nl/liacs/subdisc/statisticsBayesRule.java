...
public class statisticsBayesRule {
	/**
	 For a subgroup description (condition list with its corresponding bitsets),
	 the statistics to calculate Bayes Rule
	 */
	private BitSet itsX;
	...
	public statisticsBayesRule(BitSet theX, BitSet theTarget){
		...
		for (int i=0;i<itsX.size();i++) {
			if (itsX.get(i)){ // point in the data is a 'member' of subgroup description', so P(X|T) estimate is P(X=1|T)
				probabilitiesDataPXGivenT[i] = itsPXGivenT;
				probabilitiesDataPXGivenNotT[i] = itsPXGivenNotT;
			} else { //point in the data is not a member of subgroup description so P(X|T) is P(X=0|T)
