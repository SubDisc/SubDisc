...
/**
 * the global information contains the variables and conditions 
 * that are known to correlate with the target
 * This knowledge is captured in two lists. One with explanatory variables (as columns),
 * as well as known attribute-value conditions that correlate with the target.
 * This knowledge can be ``transformed'' into bitsets.  
 * This is useful for including these bitsets into a prediction model (i.e. logistic regression). 
 * For now we assume the explanatory variables to be ''binned'' so they can be represented as subgroup descriptions.
 */
	...
	*/
	...
	public Set<BitSet> getBitSets(){ //Use HashSet or List??
